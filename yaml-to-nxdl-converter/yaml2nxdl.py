...
"""
Parser to convert from yaml FAIRMAT internal schema to nxdl,
official NIAC format for NeXus application definitions.

Required argument: -n <filename>

Example conversion from yaml to nxdl:
    python ./yaml-to-nxdl-converter/yaml2nxdl.py -n ./ARPES/Base_Classes/NXuser_converted.yaml
Generates:
    ./ARPES/Application_Definitions/NXmpes_core_draft.yml.nxdl.xml
"""

def main(args):

    args.filename = os.path.join(*args.filename.split("/"))
    print("readfile_path: " + args.filename)
    fnm=args.filename
